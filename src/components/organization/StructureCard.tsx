  ...
  return (
    <Card
      className={classNames(
        ...
        expanded
          ? `max-h-[${EXPANDED_HEIGHT}px]`
          : `max-h-[${COLLAPSED_HEIGHT}px]`,
        !expanded && "hover:bg-muted",
      )}
      style={{
        // Only transition max-height and background, NOT border-radius
        transition:
          "max-height 0.3s cubic-bezier(0.4,0,0.2,1), background 0.3s",
      ...
      {expanded && (
        <>
          <div className="text-center mt-2 mb-4">
            <span className="text-medium pl-4 gap-8 py-2">
              Top 5 vulnerable {type}
            </span>
          </div>
          {topEntries.map((entry, index) => {
            ...
            return (
              <div
                ...
                className="hover:bg-muted flex items-center justify-between gap-8 py-2 border-t"
                onClick={(e) => {
                  e.stopPropagation();
                  const detailHref =
                    type === "Projects"
