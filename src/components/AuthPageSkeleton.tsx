// Copyright 2026 L3montree GmbH and the DevGuard Contributors.
// SPDX-License-Identifier: 	AGPL-3.0-or-later

import Image from "next/image";
import Footer from "./misc/Footer";
import FourSideGridPattern from "./misc/FourSideGridPattern";
import FlowSkeleton from "./ory/FlowSkeleton";
import { Card, CardContent } from "./ui/card";

// Shell shared by the login/registration/recovery/verification pages, holding
// their geometry until the Ory flow arrives.
export default function AuthPageSkeleton() {
  return (
    <div className="relative flex min-h-screen flex-col bg-background">
      <FourSideGridPattern />
      <div className="flex min-h-screen items-center justify-center flex-col pt-6">
        <div className="w-full max-w-6xl">
          <Card className="overflow-hidden p-0">
            <CardContent className="grid p-0 md:grid-cols-5">
              <div className="flex flex-col justify-center p-8 col-span-2">
                <div className="mb-6 flex justify-center">
                  <Image
                    className="hidden h-16 w-auto dark:block"
                    src="/logo_inverse_horizontal.svg"
                    alt="DevGuard by l3montree Logo"
                    width={200}
                    height={200}
                  />
                  <Image
                    className="h-16 w-auto dark:hidden"
                    src="/logo_horizontal.svg"
                    alt="DevGuard by l3montree Logo"
                    width={200}
                    height={200}
                  />
                </div>

                <FlowSkeleton />
              </div>

              <div
                className="col-span-3 relative hidden border-l md:block"
                style={{ background: "hsl(var(--harbor-background))" }}
                id="container-yard-scene"
              />
            </CardContent>
          </Card>
        </div>
        <div className="mt-14">
          <Footer />
        </div>
      </div>
    </div>
  );
}
