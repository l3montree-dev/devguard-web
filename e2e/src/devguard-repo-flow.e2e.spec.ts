...
describe('DevGuard repo flows', () => {
  ...
  test('test if we can add artifact manually', async ({ page }) => {
    ...
    await devguardPOM.createNewArtifact('Test Artifact','http://github.com/user-attachments/files/23216827/vex_l3montree_web_reopened.json');
    ...
    await devguardPOM.deleteFirstArtifact();
    await page.getByRole('heading', { name: 'No Artifacts Available' }).isVisible();
  });
