...
export class AdvisoryHelper {
  ...
  async createNewAdvisory(title: string, description: string, vector: string, ecosystem: string, packageName: string, semverStart: string, semverEnd: string) {
    ...
    await this.page.getByRole('textbox', { name: 'editable markdown' }).getByRole('paragraph').fill(description);
    ...
    await this.page.getByTestId("packageName-security-advisory").fill(packageName);
    await this.page.getByTestId("semverStart-security-advisory").click();
    await this.page.getByTestId("semverStart-security-advisory").fill(semverStart);
