...
export class OpenCodePOM {
  ...
  async deleteRepo(userName: string, repoName: string) {
    await this.page.goto(
      "https://gitlab.opencode.de/" + userName + "/" + repoName,
    );
    ...
    await this.page
      .getByRole("textbox", { name: "Usuari o adreça electrònica" })
      .click();
    ...
    await this.page
      .getByTestId("confirm-name-field")
      .fill(userName + "/" + repoName);
  ...
  async createRepo(repoName: string) {
    ...
    await this.page
      .getByRole("link", { name: "Create blank project Create a" })
      .click();
    await this.page
    ...
    await this.page
      .getByRole("button", { name: "‎Pick a group or namespace‎" })
      .click();
    await this.page
      .getByTestId("listbox-item-gid://gitlab/Namespaces::UserNamespace/11826")
      .getByText(userName)
      .click();
    await this.page.getByRole("button", { name: "Create project" }).click();
  }
  ...
  async verifyOnOpenCodeLoginURL() {
    await expect(this.page).toHaveURL(
      new RegExp(
        "^https://keycloak.opencode.de/auth/realms/osr/protocol/openid-connect/auth",
      ),
      { timeout: 15_000 },
    );
  }
  ...
  async login(shouldRedirect: boolean = false) {
    ...
    await this.page
      ...
      .fill(envConfig.openCode.username);
    await this.page.getByRole("textbox", { name: "Password" }).click();
    await this.page
      .getByRole("textbox", { name: "Password" })
      .fill(envConfig.openCode.password);
    ...
    do {
      ...
      const invalidAuthText = this.page.getByText(
    ...
    if (
      await this.page
        .getByRole("button", { name: "Only necessary Cookies" })
        .isVisible()
    ) {
      await this.page
        .getByRole("button", { name: "Only necessary Cookies" })
  ...
  async revokeAppAccess() {
    ...
    if (
      !(await this.page
        .getByText("You don't have any authorized applications.")
        .isVisible())
    ) {
